...
class Player(FirstPersonController):
    ...
    def __init__(self, **kwargs):
        ...
        self.model = f'{MODEL_PATH}\\steve.fbx'
        self.texture = character_texture
        self.origin_y = 0.5
        # self.color = color.black
        # self.highlight_color = color.black
        self.scale = (0.5, 1, 0.5)
        self.origin = (0, -2, 0)

        print(character_texture)

    def input(self, key):
        super().input(key)
        if key == 't':
            # 3rd person view
            self.camera_pivot.z_setter(-5)
            self.camera_pivot.y_setter(5)
...
class PlayerRepresentation(Entity):
    ...
    def __init__(self, position = (5,5,5)):
        ...
        print("PlayerRepresentation created")
